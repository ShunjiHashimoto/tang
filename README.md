...
```
#### モータ単体の動作確認
モータ単体の動作確認は、/tang/test/に書かれたスクリプトを使います。
以降のコマンドは、testディレクトリ配下で実行してください。
```bash
$ cd ~/ros1_ws/src/tang/test
```
##### エンコーダ値が正常に読み取れているか確認
左右のモータの回転数を正常に計測できているか確認します。  
```bash
$ python3 test_check_encoder_val.py {duty比}
```
{duty比}には0~100の値を設定してください。100の場合は最大速度でモータが回転するので注意してください。例えば、duty比を20にした場合は下記のとおりです。  
```bash
$ python3 test_check_encoder_val.py 20
```
出力は下記のとおりです。  
```bash
…
...
…
```
もし、左右の車輪でエンコーダ値が0の場合は、接触不良などで正常に値を読み取れていない可能性があるため、エンコーダの信号線が正常に接続されているか確認してください。  
##### 指定した速度に追従しているか確認
実行方法は下記のとおりです。  
```bash
python3 test_vel_control.py
```
速度は、/tang/test/test_config.pyのv_targetで設定できます。 
予め決められた回転数にモータの回転数が到達すればモータは停止します。 
```python
class Control:
    …
    # 目標角速度
    ...
    …
```
プログラム終了時は、/tant/test/graph/配下にグラフが保存されます。このグラフから目標速度にロボットの速度が追従できているか確認することができます。  
<img width="600" src="doc/images/target_vel.png">  

予め決められた目標速度から速度を変更したときのグラフを確認したい場合は、下記のコマンドを実行します    
```bash
python3 test_changing_vel_control.py
```
